 ...
 *   1. dequantize()    Eint8 ↁEfloat32 using per-head output quant params
 *   2. For each head, each grid cell, each anchor:
 *        sigmoid(tx,ty) ↁEcentre offsets
 *        exp(tw,th) ÁEanchor_wh ↁEbox size
 *        sigmoid(objectness) ÁEsigmoid(class_logits[argmax]) ↁEscore
 *   3. score_filter()  Ecombined score > thresh
 *   4. undo_letterbox()  Emap model coords back to original image pixels
 *   5. nms_class()     Egreedy IoU NMS per class
 *
 * All buffers are statically allocated  Eno malloc/free.
 ******************************************************************************
 */

#include "yolo_fastest_postprocessing.h"
...
/* Head 0: stride-32 (large objects)  Eanchors from yolo-fastest-1.1.cfg mask 3,4,5 */
...
/* Head 1: stride-16 (small objects)  Eanchors from yolo-fastest-1.1.cfg mask 0,1,2 */
 ...
 *
 * @param[in]  p_raw_head       Flat int8 output tensor [grid_h ÁEgrid_w ÁE255].
    ...
    for (_row = 0U; _row < grid_h; _row++)
    {
        for (_col = 0U; _col < grid_w; _col++)
        {
            ...
            for (_a_idx = 0U; _a_idx < MODEL_NUM_ANCHORS_PER_CELL; _a_idx++)
            {
                ...
                /* Find argmax class  Ework in int8 domain (monotonic) */
                ...
                /* Combined score = objectness ÁEclass confidence */
...
static int32_t apply_nms_for_class(raw_det_t           *p_raw_dets,
                                   ...
                                   float                nms_thresh,
                                   const yolo_fastest_letterbox_params_t *p_letterbox_params,
                    ...
                    Detection_t            *p_out_dets)
{
    ...
    /* Head 0: stride-32, 10ÁE0 grid, large-object anchors */
    ...
    /* Head 1: stride-16, 20ÁE0 grid, small-object anchors */
    ...
    /* ── Step 2: per-class NMS ↁEoriginal image coordinates ───────────── */
