...
static sensor_reg_t ov5640_mipi[] =
{
 ...
 /* AEC */
 { AEC_PK_MANUAL_REG, 0x00 }, { 0x3500, 0x00 }, { 0x3501, 0xFF }, { 0x3502, 0x00 },
...
fsp_err_t camera_write_array (sensor_reg_t const * p_array)
{
    ...
    while (CONFIG_TABLE_END_DETECT != p_array->reg)
    {
        if(p_array->reg != REQUEST_SOFTWARE_WAIT)
        {
            ...
            if (FSP_SUCCESS != err)
            {
                APP_PRINT("\r\n[ERR] In Function: %s(), OV5640 write failed: register=0x%04x, value=0x%02x\r\n",
                          __FUNCTION__, p_array->reg, p_array->val);
                return err;
            }
            ...
            if (FSP_SUCCESS != err)
            {
                APP_PRINT("\r\n[ERR] In Function: %s(), OV5640 read failed: register=0x%04x\r\n",
                          __FUNCTION__, p_array->reg);
                return err;
            }
            ...
            {
                APP_PRINT("\r\n[ERR] In Function: %s(), OV5640 read-back mismatch: "
                          "register=0x%04x, expected=0x%02x, actual=0x%02x\r\n",
                          __FUNCTION__, p_array->reg, p_array->val, value);
